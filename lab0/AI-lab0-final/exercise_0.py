...
def q1(shape):
    ...
    '''
    x = balabala
    x=np.zeros(shape,dtype=np.float32) #定义输出数据类型dtype
    return x
...
def q5():
    ...
    '''
    MAX=1<<16
    x=np.random.uniform(-1,1,MAX)
    y=np.random.uniform(-1,1,MAX)
    inside=x**2+y**2<=1
    num=np.sum(inside)
    res=4*(num/MAX)
    return res
        ...
        for y in range(self.height):
            ...
            for x in range(self.width):
                if (y,x) in self.walls:
                    ...
                    str+="."
                elif (y,x) == self.pacman_pos:
                    str+="P"
                else:
                    str+=" "
            recover.append(str)
        # 如果你希望返回一个多行的字符串，可以使用 join() 来将列表合并
        return '\n'.join(recover)
...
def q6(file_path):
    ...
    '''
    map=Layout.build_layout(file_path)
